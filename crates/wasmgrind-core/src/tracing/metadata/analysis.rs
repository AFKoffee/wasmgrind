...
pub fn line_sweep_algorithm<'a, I: IntoIterator<Item = &'a MemoryRecord>>(
    metadata: I,
) -> HashSet<(&'a MemoryRecord, &'a MemoryRecord)> {
    ...
    for event in events {
        match event.ty {
            IntervalEventType::Start => {
                assert!(
                    active.insert(event.interval),
                    "Interval should not be present here!"
                );
                ...
                for interval in active
                    .iter()
                    .filter(|interval| ***interval != *event.interval)
                {
            ...
            IntervalEventType::End => {
                assert!(
                    active.remove(&event.interval),
                    "Interval should always be present here!"
                );
            }
        }
    }
